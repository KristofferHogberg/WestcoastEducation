...
namespace Westcoast_Education_Api.Controllers
{
    ...
    public class StudentController : ControllerBase
    {
        ...
        [HttpGet("{id}")]
        public async Task<ActionResult<StudentViewModel>> ListStudentByIdAsync(int id)
        {
            return Ok(await _repository.GetStudentByIdAsync(id));
        }
        ...
        public async Task<ActionResult> AddUserAsync(PostStudentViewModel model)
        {
            ...
            if (result.Succeeded)
            {
                return StatusCode(201, $"User created: {model.Email}");
            }
            else
            {
